...
namespace Microsoft.MixedReality.Toolkit.Experimental.UI
{
    ...
    [AddComponentMenu("Scripts/MRTK/Experimental/Dock/NoRotationDockable")]
    public class NoRotationDockable : MonoBehaviour
    {
        [Experimental]
        ...
        private NoRotationDockPosition dockedPosition = null;
        private Vector3 dockedPositionScale = Vector3.one;

        private HashSet<NoRotationDockPosition> overlappingPositions = new HashSet<NoRotationDockPosition>();
        ...
        void Start(){
            StartCoroutine("ContinousRotation", 2.0f);
        }

        private IEnumerator ContinousRotation() {
            while (true)
            {
                if (dockingState == DockingState.Docked)
                {
                    yRotation += (Time.deltaTime * 45.0f) % 360.0f;
                    this.gameObject.transform.GetChild(0).transform.rotation = Quaternion.Euler(0, yRotation, 0);
                }
                yield return 0;
            }
        }
        ...
        public void Update()
        {
            ...
            if (isDragging && overlappingPositions.Count > 0)
            {
                ...
                {
                    closestPosition.GetComponentInParent<NoRotationDock>().TryMoveToFreeSpace(closestPosition);
                }
            }
            ...
            if (dockingState == DockingState.Docked || dockingState == DockingState.Docking)
            {
                ...
                bool isDockVertical = dockedPosition.GetComponentInParent<NoRotationDock>().isVertical;
        ...
        public void Dock(NoRotationDockPosition position)
        {
            if (!CanDock)
            {
        ...
        {
            var dockPosition = collider.gameObject.GetComponent<NoRotationDockPosition>();
        ...
        private void OnManipulationEnded(ManipulationEventData e)
        {
            ...
            if (overlappingPositions.Count > 0 && CanDock)
            {
                ...
                if (closestPosition.IsOccupied)
                {
                    if (!closestPosition.GetComponentInParent<NoRotationDock>().TryMoveToFreeSpace(closestPosition))
                    {
                        return;
                    }
                }
        ...
        private NoRotationDockPosition GetClosestPosition()
        {
            var bounds = gameObject.GetComponent<Collider>().bounds;
            var minDistance = float.MaxValue;
            NoRotationDockPosition closestPosition = null;
